...
`--privileged --cgroupns=host`. Source: the stage-2 branch staged without
`.git`, re-initialized as a fresh repo inside the container. Script:
`ops/isolated-acceptance/stage2-acceptance.sh` (unmodified run).

Result: **48/48 assertions PASS**, final line
`ALL-STAGE2-ISOLATED-ACCEPTANCE-PASSED`, exit 0, no drop-ins beyond the
container's own isolation config. Key observed behaviors:
...
- S2 drill: corrupt-state deploy failed (rc=1) → OnFailure notifier ran →
  ...
  against the drill invocation) → `request verify` completed (rc=0), state
  idle, hold kept.
- S3: `request unpause` cleared hold and re-enabled the deploy timer; the
  A→B release (requirements change) ran drain→quiesce→apply→re-exec→verify
  end to end (rc=0): recovery copy kept (A-era marker in, B-era marker
  out on rollback), checkout at B, `.deploy-done` alias == verified commit,
...
- S8 rollback: after the verified A→B release, `request rollback` returned
  the checkout, `.deploy-done`, and `verified_commit` to A, restored the
  venv from the recovery copy (B-era marker gone, A-era marker present),
  kept the hold, and left no B-era files in the checkout.

Local unit suites the same day: `pytest -q -m "not docker and not e2e"` →
1421 passed / 0 failed / 44 deselected. Not covered here (production-only
per the stage-2 spec §9): B1 real bench, B12 real rollback on the VPS,
B14 real Discord reception, B20 production bootstrap, §8.1 VM reboot
drills (T1–T6).
