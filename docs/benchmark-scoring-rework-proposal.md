...
1. Parse `trajectory` whether stored as a list or JSON string. Preserve leading dataset `system` messages. Read their nonempty `system_prompt` field, falling back to `text` and then `content`; reject malformed content or system entries after the conversation starts.
...
4. Render the message list once with the campaign's pinned chat template and `add_generation_prompt=true`. Count the final rendered input with the matching pinned tokenizer, including the dataset system prompt, role markers and any template-generated instructions. This total determines eligibility for the 4K, 8K, 16K and 32K input tiers. Match the engine's special-token handling so markers are not added twice.
...
For the pinned Qwen3.8-27B template, `true` opens a `<think>` block; `false` supplies an empty, closed block before generation. Enabling thinking also uses the template's default `xhigh` reasoning instruction. Preserve the dataset system prompt and count both it and the template-generated instruction in the final rendered input length. See the [pinned template](https://huggingface.co/Qwen/Qwen3.8-27B-FP8/blob/017b9c7af6b5689d5dd426a76e0bc077eb5ca20a/tokenizer_config.json).
