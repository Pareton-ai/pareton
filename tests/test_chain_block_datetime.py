...
def test_fetch_block_datetime_reads_sdk_timestamp():
    ...
    subtensor = SimpleNamespace(
        block_info=lambda block: SimpleNamespace(
            timestamp=expected if block == 42 else None
        )
    )
...
def test_fetch_block_datetime_fails_closed_for_numeric_timestamp():
    subtensor = SimpleNamespace(
        block_info=lambda _block: SimpleNamespace(timestamp=1_700_000_000_000)
    )
