...
def test_trace_covers_lengths_with_one_frozen_arrival_schedule(interval):
    sampled = sample(rule=rule(request_interval_ms=interval))
    ...
    assert [r.arrival_offset_ms for r in trace.requests] == [
        i * interval for i in range(8)
    ]
    assert sorted(r.input_tokens for r in trace.requests) == [
        32,
        32,
        64,
        64,
    ...
    assert len(set(sampled.row_indices)) == 8
    assert [r.input_tokens for r in trace.requests] != sorted(
        r.input_tokens for r in trace.requests
    )
    assert sample(rule=rule(request_interval_ms=interval)).body == sampled.body
