...
def _require_host_port_access() -> None:
    ...
    if Path("/.dockerenv").exists() and "Docker Desktop" in operating_system:
        pytest.skip(
            "Desktop publishes ports on the desktop host, outside this Linux VM; "
            "run published-port tests on the host. Bridge tests cover the GPU path."
        )
...
def test_publish_port_lifecycle_healthy_and_cleanup(
    ...
    _require_host_port_access()
    run_id = "itpub" + uuid.uuid4().hex[:7]
    logs = tmp_path / "logs"
